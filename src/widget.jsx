...
export default function AudicoQuoteWidget() {
  ...
  const handleSend = async () => {
    ...
    if (nextStep >= questions.length) {
      try {
        const res = await fetch(`https://audico-api-gpt.onrender.com/search_gpt?query=${encodeURIComponent(input)}&mode=${mode}`);
        const data = await res.json();

        if (data?.quote?.length > 0) {
          setQuoteItems(data.quote);
          setMessages(m => [...m, { sender: "ai", text: "Here’s what I suggest based on your request. Feel free to adjust!" }]);
        } else {
